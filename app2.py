...
# New function for Gemini AI
def get_gemini_assistance(query):
    try:
        # Add instructions for concise responses
        prompt = f"""
        Please provide a brief, concise response to this healthcare question. 
        Keep your answer under 3-4 sentences if possible, focusing on the most important information.
...
def index():
    ...
    return render_template("index2.html", places=places, address=address, error=error, ai_response=session.get('ai_response'))
